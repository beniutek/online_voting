...
class DataSigner
  ...
  # voter_id::
  #  id that the voter has passed in the requests
  # == Returns:
  ...
  def sign_vote(message, voter_id)
    ...
    admin_response = @admin_client.get_admin_signature(voter_id, blinded_encoded_encrypted_msg, voter_signed_blinded_encoded_encrypted_msg, voter_key.public_key.to_s)

    raise AdminSignatureError.new("admin response invalid") if admin_response.to_s == "" || admin_response['error']

    admin_signed_blinded_encoded_encrypted_msg = admin_response['data']['admin_signature']

    raise AdminSignatureError.new("admin signature invalid") unless admin_signer.verify(signed: admin_signed_blinded_encoded_encrypted_msg, message: blinded_encoded_encrypted_msg)
